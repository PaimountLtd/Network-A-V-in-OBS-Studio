...
static obs_source_t *find_filter_by_id(obs_source_t *context, const char *id)
{
	...
	obs_source_enum_filters(
		...
		[](obs_source_t *, obs_source_t *filter, void *param) {
			struct search_context *filter_search = (struct search_context *)param;
...
obs_properties_t *ndi_source_getproperties(void * /*data*/)
{
	...
	obs_property_t *source_list =
		obs_properties_add_list(props, PROP_SOURCE, obs_module_text("NDIPlugin.SourceProps.SourceName"), OBS_COMBO_TYPE_EDITABLE, OBS_COMBO_FORMAT_STRING);
	...
	obs_property_t *bw_modes = obs_properties_add_list(props, PROP_BANDWIDTH, obs_module_text("NDIPlugin.SourceProps.Bandwidth"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	obs_property_set_modified_callback(bw_modes, [](obs_properties_t *props, obs_property_t * /*property*/, obs_data_t *settings) {
		...
		obs_property_t *yuv_colorspace = obs_properties_get(props, PROP_YUV_COLORSPACE);
	...
	obs_property_t *sync_modes = obs_properties_add_list(props, PROP_SYNC, obs_module_text("NDIPlugin.SourceProps.Sync"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

	obs_property_list_add_int(sync_modes, obs_module_text("NDIPlugin.SyncMode.Internal"), PROP_SYNC_INTERNAL);
	obs_property_list_add_int(sync_modes, obs_module_text("NDIPlugin.SyncMode.NDITimestamp"), PROP_SYNC_NDI_TIMESTAMP);
	obs_property_list_add_int(sync_modes, obs_module_text("NDIPlugin.SyncMode.NDISourceTimecode"), PROP_SYNC_NDI_SOURCE_TIMECODE);
	...
	obs_property_t *yuv_ranges = obs_properties_add_list(props, PROP_YUV_RANGE, obs_module_text("NDIPlugin.SourceProps.ColorRange"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

	obs_property_list_add_int(yuv_ranges, obs_module_text("NDIPlugin.SourceProps.ColorRange.Partial"), PROP_YUV_RANGE_PARTIAL);
	obs_property_list_add_int(yuv_ranges, obs_module_text("NDIPlugin.SourceProps.ColorRange.Full"), PROP_YUV_RANGE_FULL);

	obs_property_t *yuv_spaces =
		obs_properties_add_list(props, PROP_YUV_COLORSPACE, obs_module_text("NDIPlugin.SourceProps.ColorSpace"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
	...
	obs_property_t *latency_modes = obs_properties_add_list(props, PROP_LATENCY, obs_module_text("NDIPlugin.SourceProps.Latency"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

	obs_property_list_add_int(latency_modes, obs_module_text("NDIPlugin.SourceProps.Latency.Normal"), PROP_LATENCY_NORMAL);
	obs_property_list_add_int(latency_modes, obs_module_text("NDIPlugin.SourceProps.Latency.Low"), PROP_LATENCY_LOW);
...
void *ndi_source_poll_audio_video(void *data)
{
	...
	while (s->running) {
		frame_received = ndiLib->recv_capture_v2(s->ndi_receiver, &video_frame, &audio_frame, nullptr, 100);

		if (frame_received == NDIlib_frame_type_audio) {
			obs_audio_frame.speakers = channel_count_to_layout(audio_frame.no_channels);

			switch (s->sync_mode) {
			case PROP_SYNC_INTERNAL:
			default:
				obs_audio_frame.timestamp = os_gettime_ns();
				obs_audio_frame.timestamp += ((uint64_t)audio_frame.no_samples * 1000000000ULL / (uint64_t)audio_frame.sample_rate);
				break;
			...
			case PROP_SYNC_NDI_SOURCE_TIMECODE:
				obs_audio_frame.timestamp = (uint64_t)(audio_frame.timecode * 100.0);
				break;
			}
		...
		if (frame_received == NDIlib_frame_type_video) {
			...
			case PROP_SYNC_NDI_TIMESTAMP:
				obs_video_frame.timestamp = (uint64_t)(video_frame.timestamp * 100);
				break;

			case PROP_SYNC_NDI_SOURCE_TIMECODE:
				obs_video_frame.timestamp = (uint64_t)(video_frame.timecode * 100);
				break;
			}
			...
			video_format_get_parameters(s->yuv_colorspace, s->yuv_range, obs_video_frame.color_matrix, obs_video_frame.color_range_min,
...
void ndi_source_update(void *data, obs_data_t *settings)
{
	...
	if (s->alpha_filter_enabled) {
		obs_source_t *existing_filter = find_filter_by_id(s->source, OBS_NDI_ALPHA_FILTER_ID);

		if (!existing_filter) {
			obs_source_t *new_filter = obs_source_create(OBS_NDI_ALPHA_FILTER_ID, obs_module_text("NDIPlugin.PremultipliedAlphaFilterName"), nullptr, nullptr);
	...
	s->yuv_range = prop_to_range_type((int)obs_data_get_int(settings, PROP_YUV_RANGE));
	s->yuv_colorspace = prop_to_colorspace((int)obs_data_get_int(settings, PROP_YUV_COLORSPACE));
