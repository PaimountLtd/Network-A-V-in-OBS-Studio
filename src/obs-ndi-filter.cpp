...
obs_properties_t *ndi_filter_getproperties(void * /*data*/)
{
	...
	obs_properties_add_text(props, FLT_PROP_NAME, obs_module_text("NDIPlugin.FilterProps.NDIName"), OBS_TEXT_DEFAULT);
...
void ndi_filter_offscreen_render(void *data, uint32_t /*cx*/, uint32_t /*cy*/)
{
	...
	if (gs_texrender_begin(s->texrender, width, height)) {
		...
		if (s->known_width != width || s->known_height != height) {
			...
			video_output_connect(s->video_output, nullptr, ndi_filter_raw_video, s);
		...
		if (video_output_lock_frame(s->video_output, &output_frame, 1, os_gettime_ns())) {
			if (s->video_data) {
			...
			for (uint32_t i = 0; i < s->known_height; ++i) {
				...
				memcpy(output_frame.data[0] + dst_offset, s->video_data + src_offset, linesize);
			}
...
struct obs_audio_data *ndi_filter_asyncaudio(void *data, struct obs_audio_data *audio_data)
{
	auto s = (struct ndi_filter *)data;
	...
	size_t data_size = audio_frame.no_channels * audio_frame.channel_stride_in_bytes;
	uint8_t *ndi_data = (uint8_t *)bmalloc(data_size);

	for (int i = 0; i < audio_frame.no_channels; ++i) {
		memcpy(&ndi_data[i * audio_frame.channel_stride_in_bytes], audio_data->data[i], audio_frame.channel_stride_in_bytes);
	}
